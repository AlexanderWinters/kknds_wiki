...
You can then use the values to monitor and track your router. Check out [this](/docs/home#monitor-router-network-traffic) on how to monitor your network traffic from Home Assistant. It will be useful to also install a MIB file.
...
### Drop WAN ping-requests

If you have services behind a VPN or a tunnel, then pinging your home network is not needed which means we can block it.
OpenWRT already has a firewall rule to allow ping requests from the WAN interface, so simply editing the rule and changing from `ACCEPT` to `DROP`.
In UCI:
```bash
uci add firewall rule
uci set firewall.@rule[-1].name='Drop-Ping'
